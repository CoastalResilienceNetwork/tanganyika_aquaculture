<template>
  <div :class="displayClass"  @click.stop @keypress.stop>
    ...
    <div class="q-pa-none">
      ...
      >
        <q-card class="">
            <q-scroll-area v-if="prop.node.description" style="height:50px; width:90%" :thumb-style="{background: 'var(--q-secondary)', width: '4px'}" >
             <div class="q-mr-md">
          ...
          <div class='row items-left q-pa-sm'>
            ...
            <div class="col-11 q-pr-xl"><q-slider  color="secondary" snap dense @change="setTransparency($event, prop.node.id, prop.node.type)" :min="0" :max="1" :step=".1" :model-value=".5" label /></div>
          </div>
        </q-card>
      </q-expansion-item>
      </q-list>
...
export default {
  ...
  watch: {
    ...
    setTicked(){
      ...
      this.$store.commit('updateTreeState', {tickedObj: tickedObj, ticked: ticked, expanded: this.expanded})
    },
    clearLayers(){
      this.ticked=[]
    }
